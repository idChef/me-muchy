...
<nav class="z-20 fixed top-0 left-0 w-full bg-gray-100 shadow-md">
	<div class="flex justify-between items-center bg-white py-2  px-10">
		...
		<ul class="flex items-center space-x-10">
			{#if $currentUser}<a href="/submit" class="btn hidden md:block"> Dodaj mema </a>{/if}
			<div class="relative flex select-none" use:clickOutside on:outclick={handleMenuClose}>
				<img
					src="/icons/user.svg"
					class="w-6 h-6  grayscale object-fill hover:cursor-pointer hidden lg:block"
				...
				<div class="lg:hidden">
					<svg
						...
						class="h-10 w-10 cursor-pointer relative z-50"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
						on:click={handleMenuToggle}
				...
				>
					...
					{:else}
						<span
							class="block px-4 py-2 mt-2  bg-transparent rounded-lg  text-md md:mt-0 focus:outline-none focus:shadow-outline select-none font-bold overflow-hidden text-ellipsis"
							>u/{$currentUser.userName}</span
						>
						<a href="/submit" class="btn" on:click={handleMenuClose}> Dodaj mema </a>
						<span
							class="block px-4 py-2 mt-2  bg-transparent rounded-lg  text-md font-semibold md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline cursor-pointer select-none"
							on:click={handleLogout}>Wyloguj</span
						>
					{/if}
				</div>
				<div
					class="px-2 py-2 bg-white rounded-md shadow absolute right-0 top-8 transition-opacity max-w-xs text-right hidden md:block"
					class:hidden-element={userMenuHidden}
				>
					{#if !$currentUser}
						<a
